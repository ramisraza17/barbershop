...
module Shopify
  module PaymentMethod
    class DeletePaymentMethod
      ...
      def self.call(*args)
        new(*args).delete_payment_method
      end

      def delete_payment_method
        url = URI("https://#{shopify_store_name}.myshopify.com/admin/api/2022-01/graphql.json")
        query = "mutation customerPaymentMethodRevoke($customerPaymentMethodId: ID!){ customerPaymentMethodRevoke(customerPaymentMethodId: $customerPaymentMethodId) { revokedCustomerPaymentMethodId, userErrors { field, message } } }"
        variable = {
