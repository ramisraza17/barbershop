...
module Shopify
  module SellingPlanGroup
    class ProductVariantLeave
      ...
      def self.call(*args)
        new(*args).leave_selling_plan_group
      end

      def leave_selling_plan_group
        url = URI("https://#{shopify_store_name}.myshopify.com/admin/api/2022-01/graphql.json")
        query = "mutation productVariantLeaveSellingPlanGroups($id: ID!, $sellingPlanGroupIds: [ID!]!) { productVariantLeaveSellingPlanGroups(id: $id, sellingPlanGroupIds: $sellingPlanGroupIds) { productVariant { id displayName } userErrors { field message } } }"
        variables = {
